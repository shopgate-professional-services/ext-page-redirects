...
export default (subscribe) => {
  if (redirectsConfig && redirectsConfig.length) {
    subscribe(appWillStart$, async ({ dispatch }) => {
      dispatch(receiveConfig(redirects));
    ...
    subscribe(appWillStart$, async ({ dispatch }) => {
      const redirectsConfiguration = await dispatch(fetchConfig());
      const groupedByPattern = redirectsConfiguration.config.reduce((acc, config) => {
        if (!acc[config.pattern]) {
          acc[config.pattern] = [];
        }
        // eslint-disable-next-line no-param-reassign
